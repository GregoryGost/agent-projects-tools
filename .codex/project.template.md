...
This template is not a rule. Keep it outside `.codex/rules/` so agents do not load it as an active rule file.

Do not keep project-specific values in portable `.codex/rules/` or reusable `.agents/skills/` files. `CODEX_PROJECT.md` is the project-specific source of truth.
...
List only profiles that apply to this repository. Delete inactive profiles.

Common profile names used by this ruleset:
...
List only rule files that should normally be applied for this project. Use paths relative to repository root.

- Core/project rules: `<.codex/rules/*.md list or none>`
- Formatting/linting rules: `<prettier_formatting.md / eslint_typescript.md / none>`
- Language rules: `<typescript_core.md / typescript_testing.md / python.md / none / other>`
...
- External-system rules: `<obsidian / jira / none / other>`
...
- Language skills: `<typescript-core / typescript-testing / python-core / none / other>`
...
Use only project-declared commands. Use `none` when the project does not provide a command.
...
- Node.js service E2E tests: `<command or none>`
- Vue Playwright E2E tests: `<command or none>`
...
- Generated/vendor/build artifacts formatting policy: `<exclude / project-specific / none>`
...
Use only for non-browser Node.js services: APIs, CLIs, workers, queue consumers, scheduled jobs, or mixed services.
...
- Active Vue rule: `<vue_frontend.md / none>`
- Active Vue skill: `<vue-expert / none>`
- Vue Router enabled: `<yes/no>`
- Vue Router rule/skill: `<vue_router.md + vue-router-expert / none>`
- Pinia enabled: `<yes/no>`
- Pinia rule/skill: `<pinia.md + pinia-expert / none>`
- VueUse enabled: `<yes/no>`
- VueUse rule/skill: `<vueuse.md + vueuse-expert / none>`

## Styling Profile

Enable only the styling systems that the project actually uses.
...
- Wait target policy: `<specific locator/state required / project-specific>`
- Screenshot/artifact path: `<path / none>`
- Console/network summary required: `<yes/no>`
...
- Active Vue testing rule/skill: `<vitest_vue_testing / none / project-specific>`
...
- Auth/storage state policy: `<policy / none>`
- Backend/API readiness policy: `<policy / none>`
- Trace/screenshot/video policy: `<policy / none>`
...
- E2E command: `<command / none>`
- Test environment: `<node / project-specific>`
- Migrations/seeds policy: `<policy / none>`
- Test data isolation policy: `<unique IDs / disposable DB / cleanup hooks / project-specific>`
