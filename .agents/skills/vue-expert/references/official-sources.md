# Vue Expert Official Sources

Use these sources when a Vue, Vite, or Vue TypeScript rule needs verification.

- Vue Guide: https://vuejs.org/guide/introduction.html
- Vue Single-File Components: https://vuejs.org/guide/scaling-up/sfc.html
- Vue Composition API FAQ: https://vuejs.org/guide/extras/composition-api-faq.html
- Using Vue with TypeScript: https://vuejs.org/guide/typescript/overview.html
- TypeScript with Composition API: https://vuejs.org/guide/typescript/composition-api.html
- Vue Tooling: https://vuejs.org/guide/scaling-up/tooling.html
- Vite Guide: https://vite.dev/guide/
- Vite Config: https://vite.dev/config/
- Vite Env Variables and Modes: https://vite.dev/guide/env-and-mode
- @vitejs/plugin-vue: https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue

Prefer official Vue and Vite docs over project examples when resolving framework behavior. Project conventions still come from `CODEX_PROJECT.md` and the target repository.
